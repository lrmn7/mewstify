...
export default class implements PrefixCommand {
  ...
  async run(
    ...
    const botMember = message.guild!.members.cache.get(client.user!.id); // Get the bot's member in the guild
    if (
      !channel ||
      (botMember?.voice.channel && channel.id !== botMember.voice.channel.id)
    )
      return msg.edit({
        embeds: [
          new EmbedBuilder()
